...
class TestLoginCourier:
    ...
    def test_courier_login_success(self, courier):
        ...
        with allure.step("Проверить, что авторизация прошла успешно (статус 200, получен id)"):
    ...
    def test_courier_login_without_parameters_failed(self, courier_data):
        with allure.step(f"Отправить запрос на авторизацию с неполными данными: {courier_data}"):
            response = requests.post(f'{Urls.QA_SCOOTER_URL}{Endpoints.login_courier}', data=courier_data)
        with allure.step("Проверить, что сервер вернул ошибку 400 с сообщением 'Недостаточно данных для входа'"):
    ...
    def test_courier_login_without_null_login_failed(self):
        with allure.step(f"Отправить запрос на авторизацию с несуществующими данными: {DataCourier.null_data_login}"):
            response = requests.post(f'{Urls.QA_SCOOTER_URL}{Endpoints.login_courier}', data=DataCourier.null_data_login)
        with allure.step("Проверить, что сервер вернул ошибку 404 с сообщением 'Учетная запись не найдена'"):
